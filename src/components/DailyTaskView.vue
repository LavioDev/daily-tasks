...
const completedCount = computed(() => dailyTaskStore.tasks.filter(task => isCompleted(task.id)).length)
const completionPercent = computed(() => dailyTaskStore.tasks.length ? Math.round((completedCount.value / dailyTaskStore.tasks.length) * 100) : 0)
...
function addTask() {
  ...
  dailyTaskStore.addTask(newTaskTitle.value)
  newTaskTitle.value = ''
}

function startEdit(task: Task) { editingTaskId.value = task.id; editingTitle.value = task.title }
function saveEdit(task: Task) { if (editingTitle.value.trim()) dailyTaskStore.updateTaskTitle(task.id, editingTitle.value); editingTaskId.value = null }
function removeTask(task: Task) { dailyTaskStore.deleteTask(task.id) }
</script>
  ...
  <main class="flex-1 bg-slate-50 px-4 py-6 sm:px-6 lg:px-10">
    <div class="mx-auto max-w-3xl">
      <section class="border border-slate-200 bg-white">
        ...
        <div v-if="dailyTaskStore.tasks.length" class="divide-y divide-slate-200">
          <article v-for="task in dailyTaskStore.tasks" :key="task.id" class="group flex min-h-16 items-center gap-3 px-5 py-3 transition-colors hover:bg-violet-50/30 sm:px-7" :class="isCompleted(task.id) ? 'bg-slate-50/70' : 'bg-white'">
